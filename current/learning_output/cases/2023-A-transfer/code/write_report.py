...
data=json.loads((ROOT/'results/RESULTS.json').read_text());A=json.loads((ROOT/'results/ANALYTIC.json').read_text())
S=json.loads((ROOT/'results/SENSITIVITY.json').read_text());base=[next(x for x in data if x['id']==f'Q{q}_base') for q in range(1,5)]
text='''# 2023 年研赛 A 题：WLAN 网络信道接入机制建模
...
'''
for q,row in enumerate(base,1):
    text+=f"- Q{q} 基础场景：总吞吐 {row['total_mean']:.4f} ± {row['ci95_halfwidth']:.4f} Mbps；各 AP 均值 "+'、'.join(f'{v:.4f}' for v in row['per_node_mean'])+f" Mbps；Jain 均值 {row['fairness_mean']:.4f}。\n"
text+='\n'
text+=f"Q1 固定点为 {A['q1']['total_mbps']:.4f} Mbps，较仿真高 {(A['q1']['total_mbps']/base[0]['total_mean']-1)*100:.2f}%。Q2 精确链为 {A['q2']['total_mbps']:.4f} Mbps，落在仿真均值区间内；稳态残差为 {A['q2']['stationary_residual']:.2e}。Q3 Poisson 闭合为 {A['q3']['total_mbps']:.4f} Mbps，较仿真高 {(A['q3']['total_mbps']/base[2]['total_mean']-1)*100:.2f}%。Q4 理想链为 {A['q4']['total_mbps']:.4f} Mbps，其中中央节点预测 {A['q4']['per_node_mbps'][1]:.4f} Mbps，明显高于事件模型的 {base[3]['per_node_mean'][1]:.4f}，说明只看理想活动集合会低估中央劣势。\n\n"
text+='![数值模型与事件模型比较](deliverables/model_comparison.png)\n\n![隐藏节点参数和三 BSS 公平性](deliverables/hidden_and_fairness.png)\n\n'
text+='### 附录6参数逐组结果\n\n每行给出速率、CWmin、重传上限；CWmax 均为 1024。\n\n'
for q in [3,4]:
    for row in [x for x in data if x['question']==q and not x['id'].endswith('base')]:
        p=row['params'];text+=f"- Q{q}：{p['rate']} Mbps，W={p['cwmin']}，r={p['retries']}：总吞吐 {row['total_mean']:.4f} ± {row['ci95_halfwidth']:.4f} Mbps；各 AP "+'、'.join(f'{v:.3f}' for v in row['per_node_mean'])+'。\n'
text+='''
...
'''
labels={'Q3_no_channel_loss':'Q3 取消独立信道丢包','Q4_all_concurrent_data_succeed':'Q4 把所有重叠数据都视作可成功','Q1_data_only_carrier':'Q1 仅数据帧作为可监听忙期','Q2_data_only_carrier':'Q2 仅数据帧作为可监听忙期','Q4_data_only_carrier':'Q4 仅数据帧作为可监听忙期'}
for row in S:text+=f"- {labels[row['id']]}：{row['total_mean']:.4f} ± {row['ci95_halfwidth']:.4f} Mbps。\n"
text+='''
...
'''
(ROOT/'REFERENCE_SOLUTION.md').write_text(text,encoding='utf-8')
print('Reference solution written')
