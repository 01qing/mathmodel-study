...
当任务来自 `graduate-mathmodel-learning` 的案例学习、优秀论文复盘或新题相似案例迁移时，先读：

- `references/evidence-role-figure-design.md`

不要按固定模板机械画图。每张建议图必须先定义 `purpose`、`evidence_role`、`cannot_prove` 与 `comparison_unit`。

优先学习优秀论文的**论证顺序和比较结构**，而不是照抄配色。2025 E 已确认的高价值结构包括：
- 同一信号的机理四联图；
- 相关性→PCA→重要性/互信息的特征筛选证据链；
- 同尺度混淆矩阵 + 重复运行箱线图；
- 迁移前后成对嵌入 + MMD/CORAL 定量指标；
- 目标概率热图（明确不是准确率）；
- 机理事前解释→迁移过程→SHAP/attention 事后解释。

可调用 `scripts/evidence_figure_templates.py` 生成这些结构的代码级模板。
