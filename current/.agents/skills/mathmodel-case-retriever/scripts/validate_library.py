...
def run():
    ...
    reviewed_train_groups={p['case_group'] for p in papers if p['split']=='train' and p.get('reviewed_pages')}
    assert set(result['candidate_pool_groups'])==reviewed_train_groups
    assert result['matches'] and any(m['case_group']=='2024-A' for m in result['matches'])
    for m in result['matches']:
        assert byid[m['paper_id']]['split']=='train'
        assert m['page'] in byid[m['paper_id']].get('reviewed_pages',[])
