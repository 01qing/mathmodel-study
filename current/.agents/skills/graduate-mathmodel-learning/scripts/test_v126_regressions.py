...
# ADAPTED v1.29+: S029 is allowed reviewed Dev-only after freeze; S030/S031 remain reserve.
p29=next(x for x in papers if x['paper_id']=='S029'); ok('S029 dev protected by boundary',p29['split']=='dev' and p29.get('dev_boundary')=='DEV_ONLY_NOT_TRAIN_RETRIEVAL')
...
ok('state preserves S026-or-later',tuple(map(int,state['current_version'].split('.'))) >= (1,26,0) and (R/'knowledge_base/paper_reviews/2025-A-S026-core.json').exists())
