...
ck('version',st['current_version']=='1.35.0'); ck('latest',st['latest_completed_paper']=='S042'); ck('R2 latest',st['reproduction_level_latest']=='S042:R2')
...
for pid in ['S043','S044','S045']: ck(pid+' unread train',P[pid]['split']=='train' and P[pid]['reviewed_pages']==[])
...
mp=load('knowledge_base/cross_paper_maps/2025-F-S042-S045.json'); ck('map provisional',mp['status']=='PROVISIONAL_ONE_OF_FOUR_TRAIN_PAPERS_REVIEWED'); ck('reviewed only S042',mp['papers_reviewed']==['S042']); ck('pending three',mp['papers_pending']==['S043','S044','S045'])
...
nexttxt=(ROOT/'learning_output/context/next_learning.md').read_text(encoding='utf-8'); ck('next S043','S043 / 2025-F' in nexttxt); ck('not due','due only after S045' in nexttxt); ck('test frozen','Test' in nexttxt and 'frozen' in nexttxt)
