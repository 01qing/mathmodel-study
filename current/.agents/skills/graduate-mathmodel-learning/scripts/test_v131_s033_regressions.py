...
ck('map reviewed includes S032 S033',mp['papers_reviewed'][:2]==['S032','S033'])
ck('map progression keeps S034 completed','S034' not in mp['papers_pending'])
ck('map legal progression after S033',mp['status'] in ['PROVISIONAL_AFTER_TWO_TRAIN_PAPERS','PROVISIONAL_AFTER_THREE_TRAIN_PAPERS','PROVISIONAL_AFTER_FOUR_TRAIN_PAPERS','FINAL_FIVE_TRAIN_PAPERS_REVIEWED'])
ck('mini transfer legal lifecycle',('NOT_DUE' in mp['mini_transfer']) or ('PASS_MECHANISM_TRANSFER' in mp['mini_transfer']))
# retriever metadata aligned
cg=json.loads((A/'case_group_maps.json').read_text(encoding='utf-8'))
ck('retriever group legal progression',cg['2025-C']['status'] in ['PROVISIONAL_2_OF_5_TRAIN_PAPERS','PROVISIONAL_3_OF_5_TRAIN_PAPERS','PROVISIONAL_4_OF_5_TRAIN_PAPERS','FINAL_5_OF_5_TRAIN_PAPERS'])
ck('retriever reviewed starts S032 S033',cg['2025-C']['reviewed_papers'][:2]==['S032','S033'])
...
ck('S034 train',P['S034']['split']=='train'); ck('S034 may be reviewed after v1.31',P['S034'].get('reviewed_pages',[]) in [[],list(range(1,69))])
