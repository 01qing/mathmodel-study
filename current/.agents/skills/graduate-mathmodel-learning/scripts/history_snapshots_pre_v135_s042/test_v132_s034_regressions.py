...
ck('version 1.32',(ROOT/'VERSION').read_text().strip()=='1.32.0')
# assets
assets=[
...
ck('map reviewed3',mp['papers_reviewed']==['S032','S033','S034'])
ck('map pending2',mp['papers_pending']==['S035','S036'])
ck('map status3',mp['status']=='PROVISIONAL_AFTER_THREE_TRAIN_PAPERS')
ck('map mini not due','NOT_DUE' in mp['mini_transfer'])
for q in ['Q1','Q2','Q3','Q4']:
...
ck('figure status3',fr['status']=='PROVISIONAL_AFTER_S032_S033_S034')
...
ck('retriever status3',cg['status']=='PROVISIONAL_3_OF_5_TRAIN_PAPERS')
ck('retriever reviewed3',cg['reviewed_papers']==['S032','S033','S034'])
ck('retriever pending2',cg['pending_papers']==['S035','S036'])
...
ck('next S035','S035 / 2025-C' in nexttxt)
ck('next dev reserve','S030/S031 remain Dev reserve' in nexttxt)
ck('next transfer due after S036','due only after S036' in nexttxt)
