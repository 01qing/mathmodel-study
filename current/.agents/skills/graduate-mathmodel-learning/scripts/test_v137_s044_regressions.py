...
ck('version compatible',tuple(map(int,st['current_version'].split('.')[:2])) >= (1,37))
ck('latest compatible',st['latest_completed_paper'] in {'S044','S045','S013','S014','S015','S016'})
ck('R2 latest compatible',st['reproduction_level_latest'] in {'S044:R2','S045:R2','S013:R2','S014:R2','S015:R2','S016:R2'})
req=[
...
ck('S045 train timepoint-compatible',P['S045']['split']=='train' and P['S045']['reviewed_pages'] in ([],list(range(1,95))))
...
ck('map timepoint-compatible',mp['status'] in {'PROVISIONAL_THREE_OF_FOUR_TRAIN_PAPERS_REVIEWED','FINAL_FOUR_OF_FOUR_TRAIN_PAPERS_REVIEWED'})
ck('reviewed prefix',mp['papers_reviewed'][:3]==['S042','S043','S044'])
ck('pending timepoint-compatible',mp['papers_pending'] in (['S045'],[]))
for rel in [
...
ck('case group timepoint-compatible',cgm['2025-F']['status'] in {'PROVISIONAL_THREE_OF_FOUR_TRAIN_PAPERS_REVIEWED','FINAL_FOUR_OF_FOUR_TRAIN_PAPERS_REVIEWED'})
...
ck('next context advanced','2025-F' in nexttxt or '2024-D' in nexttxt)
ck('group gate context','Mini Transfer' in nexttxt or 'Dev' in nexttxt)
