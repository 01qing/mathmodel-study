...
ck('version',st['current_version'] in {'1.36.0','1.37.0'}); ck('latest',st['latest_completed_paper'] in {'S043','S044'}); ck('R1 latest',st['reproduction_level_latest'] in {'S043:R1','S044:R2'})
...
for pid in ['S045']: ck(pid+' unread train',P[pid]['split']=='train' and P[pid]['reviewed_pages']==[])
...
mp=load('knowledge_base/cross_paper_maps/2025-F-S042-S045.json'); ck('map legal progression',mp['status'] in {'PROVISIONAL_TWO_OF_FOUR_TRAIN_PAPERS_REVIEWED','PROVISIONAL_THREE_OF_FOUR_TRAIN_PAPERS_REVIEWED'}); ck('reviewed S042 S043',mp['papers_reviewed'][:2]==['S042','S043']); ck('S045 pending','S045' in mp['papers_pending'])
...
nexttxt=(ROOT/'learning_output/context/next_learning.md').read_text(encoding='utf-8'); ck('next legal progression',('S044 / 2025-F' in nexttxt) or ('S045 / 2025-F' in nexttxt)); ck('not due','NOT_DUE' in nexttxt and 'S045' in nexttxt); ck('test frozen text','Test' in nexttxt and 'frozen' in nexttxt)
