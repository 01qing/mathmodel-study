...
def _build_model_for_mode(cfg, centers, device, use_preprocessed):
    ...
    in_ch_input = 1 if use_preprocessed else cfg.get("model", {}).get("in_ch_input", 2)
    model = BearingNetV2(fs_out, centers, ncls=4, in_ch_input=in_ch_input).to(device)
...
def _save_adapt_loss_csv_png(history, out_dir, lambda_coral):
    ...
    with open(csv_path, "w", newline="", encoding="utf-8") as f:
        ...
        for r in history:
            w.writerow([r["epoch"], r["loss"], r["lcls"], r["lcor"], lambda_coral,
                        r["wcor"], r["lent"], r["went"]])

    es   = [r["epoch"] for r in history]
    Ltot = [r["loss"] for r in history]
    Lcls = [r["lcls"] for r in history]
    Lcor_w = [r["wcor"] for r in history]
    Lent_w = [r["lent"]*r["went"] for r in history]
...
def adapt_coral(cfg, ckpt, out, epochs=30, bs=64, lr=5e-4, lambda_coral=0.1, shot=False, device="cuda"):
    ...
    for ep in range(1, epochs+1):
        ...
        for (xs,ys),(Xt,fn) in zip(dl_src, dl_tgt):
            ...
            Xt_bct = Xt.to(device) if use_pre else Xt.squeeze(0).to(device)
        ...
        if len(step_loss) > 0:
            rec = {
                ...
                "lcor": float(np.mean(step_lcor)),
                "lent": float(np.mean(step_lent)),
                "went": float(np.mean(step_went)),
                "wcor": float(lambda_coral * np.mean(step_lcor)),
            }
            epoch_hist.append(rec)
            print(f"[ADAPT] ep{ep:03d} loss={rec['loss']:.4f} | cls={rec['lcls']:.4f} | "
