...
class BearingNetV2(nn.Module):
    ...
    def forward(self, x):
        # 期望 x 形状：[B,C,T] / [C,T] / [T]，统一为 [B,C,T]
        x = ensure_bct(x)
        # 通道注意力
        x_se, w = self.chan_se(x)                  # w:[B,C]
        # 合成为单通道做机理滤波
        x_mono = x_se.sum(dim=1, keepdim=True)     # [B,1,T]
        y = self.fb(x_mono)                        # [B,M,T], M=len(centers)
        # 机理分组门控
        y, gate_w = self.gate(y)                   # gate_w:[groups]
        # 归一化 + 编码 + 分类
        y = self.norm(y)
        h = self.enc(y)                            # [B,128]
...
def _build_model_for_mode(cfg, centers, device, use_preprocessed):
    ...
    model = BearingNetV2(fs_out, centers, ncls=4, in_ch_input=in_ch_input).to(device)  # ← 用 V2
...
def viz_gate_channel_weights(cfg, out, device="cuda", n_src=8, n_tgt=8):
    ...
    # 保存 .npy（若非空）
    try:
        if len(gate_S)>0: np.save(os.path.join(out,"gate_weights_source.npy"), np.stack(gate_S,0))
        if len(gate_T)>0: np.save(os.path.join(out,"gate_weights_target.npy"), np.stack(gate_T,0))
        if len(chan_S)>0: np.save(os.path.join(out,"chan_weights_source.npy"), np.stack(chan_S,0))
        if len(chan_T)>0: np.save(os.path.join(out,"chan_weights_target.npy"), np.stack(chan_T,0))
        print("Saved raw weights to .npy in", out)
    except Exception as e:
        print("[WARN] saving npy failed:", e)

    def _plot_box(data, title, labels, save):
        if len(data)==0: 
            print("[INFO]", title, "no data, skip.")
            return
        arr = np.stack(data, 0)
...
if __name__=="__main__":
    ...
    # 新增 gate 阶段
    ap.add_argument("--stage", type=str, choices=["train_source","adapt","infer","viz","feat","feat_clf","viz_report","gate"], default="train_source")
    ap.add_argument("--lambda_coral", type=float, default=0.1)
    ap.add_argument("--shot", action="store_true")
    # === 关键改动：把默认输出目录改到 E:/dg/sjwl/newout ===
    ap.add_argument("--out", type=str, default=r"E:/dg/sjwl/newout")
    ap.add_argument("--feat_csv", type=str, default=r"E:/dg/sjwl/newout/source_features.csv")
    ...
    if args.stage=="train_source":
        ...
        viz_gate_channel_weights(cfg, args.out, device=args.device)

# ====== 覆盖/新增：自适应训练期间记录并绘图（整体 loss 与 lambda_coral）======

def _save_adapt_loss_csv_png(history, out_dir, lambda_coral):
    """
    根据 adapt 训练记录导出 CSV 与曲线图。
    history: list of dict(epoch, loss, lcls, lcor, lent, went, wcor)
    产物：
      - {out_dir}/adapt_loss_log.csv
      - {out_dir}/adapt_loss_curves.png
      - {out_dir}/adapt_coral_raw.png
    """
    import csv
    os.makedirs(out_dir, exist_ok=True)

    # 1) CSV
    csv_path = os.path.join(out_dir, "adapt_loss_log.csv")
    with open(csv_path, "w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
        w.writerow(["epoch","loss_total","loss_cls","loss_coral_raw","lambda_coral",
                    "weighted_coral","loss_ent_raw","ent_weight"])
        for r in history:
            w.writerow([
                r["epoch"], r["loss"], r["lcls"], r["lcor"], lambda_coral,
                r["wcor"], r["lent"], r["went"]
            ])

    # 2) 曲线：总损失 + 分类损失 + λ·CORAL + 加权熵
    es   = [r["epoch"] for r in history]
    Ltot = [r["loss"] for r in history]
    Lcls = [r["lcls"] for r in history]
    Lcor_w = [r["wcor"] for r in history]                   # λ·CORAL
    Lent_w = [r["lent"]*r["went"] for r in history]         # 加权熵

    plt.figure(figsize=(7.6,4.8))
    plt.plot(es, Ltot,  label="total loss", linewidth=2)
    plt.plot(es, Lcls,  label="cls loss", linewidth=1.8)
    plt.plot(es, Lcor_w,label=f"lambda*coral (λ={lambda_coral})", linewidth=1.8)
    plt.plot(es, Lent_w,label="entropy (weighted)", linewidth=1.8)
    plt.xlabel("epoch"); plt.ylabel("loss")
    plt.title("Adaptation loss curves")
    plt.grid(alpha=0.25); plt.legend()
    plt.tight_layout(); plt.savefig(os.path.join(out_dir,"adapt_loss_curves.png"), dpi=220); plt.close()

    # 3) 原始 CORAL（未乘 λ），便于判断量级是否合适
    plt.figure(figsize=(7.6,4.2))
    plt.plot(es, [r["lcor"] for r in history], linewidth=2)
    plt.xlabel("epoch"); plt.ylabel("raw coral loss")
    plt.title(f"CORAL loss (raw), λ={lambda_coral}")
    plt.grid(alpha=0.25)
    plt.tight_layout(); plt.savefig(os.path.join(out_dir,"adapt_coral_raw.png"), dpi=220); plt.close()

    print("Saved:",
          csv_path,
          os.path.join(out_dir,"adapt_loss_curves.png"),
          os.path.join(out_dir,"adapt_coral_raw.png"))


def adapt_coral(cfg, ckpt, out, epochs=30, bs=64, lr=5e-4, lambda_coral=0.1, shot=False, device="cuda"):
    """
    覆盖版：在原有 CORAL 迁移训练基础上，
    - 按 epoch 记录总损失与各组成项（分类 / CORAL / 熵最小化与其权重）
    - 导出 CSV 与曲线图
    - 其它行为与原实现一致（包含 AdaBN 与可选 SHOT）
    """
    set_seed(1)
    fs_out = cfg["target_domain"]["fs_hz"]
    centers = build_centers(cfg, rpm=cfg["conditions"]["rpm_source_candidates"][0], harmonics=2)
    ...
    if use_pre:
        ...
        ds_tgt = PreprocessedTargetDS(cfg)
        dl_src = DataLoader(ds_src, batch_size=bs, shuffle=True,  num_workers=0, drop_last=True)
        dl_tgt = DataLoader(ds_tgt, batch_size=bs, shuffle=False, num_workers=0)  # 预处理分支：tgt 也按 B 批
    else:
        ds_src = SourceBearingDS(cfg, "train", fs_out, max_ch=in_ch_input)
        ds_tgt = TargetBearingDS(cfg, fs_out, max_ch=in_ch_input)
        dl_src = DataLoader(ds_src, batch_size=bs, shuffle=True,  num_workers=0, drop_last=True)
        dl_tgt = DataLoader(ds_tgt, batch_size=1,  shuffle=False, num_workers=0) # 在线 MAT：tgt 是 [1]（每文件若干窗口）
    ...
    # === 训练并记录 ===
    epoch_hist = []  # 每 epoch 的均值：loss, lcls, lcor(raw), lent(raw), went, wcor

    for ep in range(1, epochs+1):
        model.train()
        step_loss, step_lcls, step_lcor, step_lent, step_went = [], [], [], [], []

        for (xs,ys),(Xt,fn) in zip(dl_src, dl_tgt):
            xs,ys = xs.to(device), ys.to(device)
            Xt_bct = Xt.to(device) if use_pre else Xt.squeeze(0).to(device)  # [B,1,T] 或 [W,C,T]
            ...
            with torch.cuda.amp.autocast(enabled=device.startswith("cuda")):
                ...
                Lcls = ce(logits_s, ys)
                Lcor = coral_loss(hs, ht)                     # 原始 CORAL（未乘 λ）
                Lent, pt = entropy_minimization(logits_t, T=1.0)
            ...
            if not torch.isfinite(loss):
                print("[WARN] skip NaN step on", (fn[0] if isinstance(fn,(list,tuple)) else fn))
                continue
            ...
            # 记录 step
            step_loss.append(float(loss.detach().cpu()))
            step_lcls.append(float(Lcls.detach().cpu()))
            step_lcor.append(float(Lcor.detach().cpu()))
            step_lent.append(float(Lent.detach().cpu()))
            step_went.append(float(w_ent))

        # 本 epoch 的均值
        if len(step_loss) > 0:
            rec = {
                "epoch": ep,
                "loss": float(np.mean(step_loss)),
                "lcls": float(np.mean(step_lcls)),
                "lcor": float(np.mean(step_lcor)),                 # 原始 CORAL
                "lent": float(np.mean(step_lent)),                 # 原始 熵
                "went": float(np.mean(step_went)),
                "wcor": float(lambda_coral * np.mean(step_lcor)),  # 加权 CORAL
            }
            epoch_hist.append(rec)
            print(f"[ADAPT] ep{ep:03d} "
                  f"loss={rec['loss']:.4f} | cls={rec['lcls']:.4f} | "
                  f"coral={rec['lcor']:.4f} (λ·coral={rec['wcor']:.4f}) | "
                  f"ent={rec['lent']:.4f} (w={rec['went']:.4f})")
        else:
            print(f"[ADAPT] ep{ep:03d} done (no valid steps).")
    ...
    # 导出日志与曲线
    if len(epoch_hist) > 0:
        _save_adapt_loss_csv_png(epoch_hist, out, lambda_coral)

    # === 可选：SHOT 自训练，保持你原来的逻辑 ===
    if not shot:
        return os.path.join(out,"model_adapt_coral.pth")
    ...
    for ep in range(1, 3):
        for Xt,fn in dl_tgt:
            Xtt = Xt.to(device) if use_pre else Xt.squeeze(0).to(device)
            logits, h, _, _ = model(Xtt)
            p = F.softmax(logits/Tt,1).clamp(1e-6, 1-1e-6)
            conf, ypl = p.max(1)
            m = conf >= tau
