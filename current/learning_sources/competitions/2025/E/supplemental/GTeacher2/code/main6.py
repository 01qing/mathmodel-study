...
class SourceBearingDS(Dataset):
    def __init__(self, cfg, split="train", fs_out=32000, seed=0, max_ch=2):
        ...
        self.labels = LABEL2ID   # 仅 OR/IR/B
    ...
    def __getitem__(self, i):
        ...
        if len(xs)==0:
            ...
            # 无法取到信号时，返回全零并给个默认标签（此处随便取 0=OR）
            return torch.zeros(1, L), 0
...
def _read_index_csv(csv_path):
    ...
    with open(csv_path, "r", encoding="utf-8") as f:
        ...
        def parse_line(first_line):
            ...
            # 第3列若是标签，则为带标签索引；否则为两列（路径,标签）
            if len(parts) >= 3 and parts[2] in set(CLASSES) | {"UNK"}:
                return True
            return False
...
_LABEL2ID = LABEL2ID  # 仅 OR/IR/B

class PreprocessedSourceDS(Dataset):
    """从 preprocessed index 读取有标签窗口用于源域 (仅 OR/IR/B)"""
    def __init__(self, cfg, split="train"):
        self.cfg = cfg
        all_rows = _read_index_csv(cfg["preprocessed"]["source_index"])
        rows = [(f, y) for (f, y) in all_rows if y in _LABEL2ID]  # 自动丢弃 N/UNK
    ...
    def __getitem__(self, i):
        ...
        return torch.from_numpy(x).unsqueeze(0), _LABEL2ID.get(yname, 0)
...
class Classifier(nn.Module):
    def __init__(self, d=128, ncls=NCLS):
        super().__init__()
...
class BearingNet(nn.Module):
    def __init__(self, fs, centers_hz, ncls=NCLS, in_ch_input=1):
        super().__init__()
...
def compute_class_weights(ds, ncls=NCLS, max_samples=2000, device="cpu"):
...
def evaluate(model, loader, device="cuda"):
    ...
    if len(Yt)==0:
        return {"macro_f1":0.0, "acc":0.0, "cm":np.zeros((NCLS,NCLS),dtype=int)}
...
def _build_model_for_mode(cfg, centers, device, use_preprocessed):
    ...
    model = BearingNet(fs_out, centers, ncls=NCLS, in_ch_input=in_ch_input).to(device)
...
def train_source(cfg, out, epochs=200, bs=64, lr=2e-3, device="cuda"):
    ...
    cls_w = compute_class_weights(ds_tr, ncls=NCLS, device=device)
...
def adapt_coral(cfg, ckpt, out, epochs=30, bs=64, lr=5e-4, lambda_coral=0.1, shot=False, device="cuda"):
    ...
    ce = nn.CrossEntropyLoss(weight=compute_class_weights(ds_src, ncls=NCLS, device=device))
    scaler = torch.cuda.amp.GradScaler(enabled=device.startswith("cuda"))
...
def extract_source_features(cfg, out_dir):
    """从源域原始 .mat 文件抽样窗口，导出包络谱相关机理特征（FBE）+ 统计量；仅 OR/IR/B"""
    import pandas as pd
...
def train_feat_classifier(feat_csv, out_dir):
    ...
    y = np.array([LABEL2ID[v] for v in df["label"].values], dtype=int)

    Xtr, Xte, ytr, yte = train_test_split(X, y, test_size=0.25, random_state=0, stratify=y)
    clf = RandomForestClassifier(n_estimators=400, max_depth=None, n_jobs=-1, random_state=0)
    clf.fit(Xtr, ytr)
    yp = clf.predict(Xte)
    cm = confusion_matrix(yte, yp, labels=list(range(NCLS)))

    joblib.dump(clf, os.path.join(out_dir,"rf_feat_model.joblib"))

    def plot_confusion(cm, out_path, labels=CLASSES, title="RF Confusion"):
...
def plot_confusion(cm, out_path, labels=CLASSES, title="Confusion Matrix"):
    plt.figure(figsize=(4.6,4.2))
    im = plt.imshow(cm, cmap="Blues")
    plt.colorbar(im, fraction=0.046, pad=0.04)
